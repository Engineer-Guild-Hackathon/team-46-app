...
export async function fetchBooks(params: FetchBooksParams = {}): Promise<BooksPage> {
...
export function fetchBooksCached(params: FetchBooksParams = {}): Promise<BooksPage> {
...
export function createPaginatedBooksStore(
  ...
  let current: BooksPage = { items: [], start: 0, size: initialParams.size ?? 20, hasMore: true }
  ...
  async function load(startOverride?: number) {
    ...
    try {
      ...
      const page = await fetchBooks({ ...initialParams, start, size: current.size })
      if (start === 0) {
        current = page
      } else {
        current = {
