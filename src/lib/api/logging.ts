...
export const FEEDBACK_ENDPOINT = 'https://us-central1-flexread-egh.cloudfunctions.net/feedback'

export interface FeedbackResult {
  result: string
}

export type FeedbackType = 'openJapanese' | 'openWord' | 'difficultBtn' | 'howWasIt'
...
function assertBase(p: BaseFeedbackParams) {
  ...
  if (p.rate === undefined || p.rate === null || p.rate === '') throw new Error('rate is required')
  if (!p.type) throw new Error('type is required')
}

export async function logFeedback(params: BaseFeedbackParams): Promise<FeedbackResult> {
  assertBase(params)
