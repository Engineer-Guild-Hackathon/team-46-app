...
import {
  fetchBooks,
  fetchBooksCached,
  createPaginatedBooksStore,
} from "../../../src/lib/api/books";
...
describe("createPaginatedBooksStore", () => {
  ...
  it("loads initial page, supports loadMore & refresh, and sets hasMore correctly", async () => {
    ...
    const unsub = store.subscribe((v) =>
      states.push(structuredClone(v) as StateShape),
    );
    ...
    await store.loadMore();
    expect(
      fetchMock.mock.calls.filter((c) => (c[0] as string).includes("start=6")),
    ).toHaveLength(0);
